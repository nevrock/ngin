...
public:
    ...
    glm::vec3 getVec(const std::string& key, const glm::vec3& defaultValue) const {
        ...
        if (l.size() != 3) {
            std::cout << "dict failed to convert to vec - nevc length mismatch" << std::endl;
...
private:
    ...
    void parseDictElement(Nevf* dict, std::string& key, std::string& value) {
    ...
    Nevf* parseDict(Nevf* dict, std::string& collectionName) {
        dict->set(collectionName, Nevf());
        return dict->get<Nevf>(collectionName);
    }
    void parseNevcElement(std::string& collectionName, std::string& value) {
        Nevc* nevc = get<Nevc>(collectionName);
        if (nevc) {
            // If the nevc exists, add the parsed value to it
            nevc->add(parseValue(value));
        } else {
            // If the nevc does not exist, create a new Nevc and add the parsed value
            Nevc newNevc;
            newNevc.add(parseValue(value));
            // Store the new Nevc in the dictionary
            data_[collectionName] = newNevc;
        }
    }
};

...
void print_job_system_diagnostics(const ngin::jobs::JobSystem& job_system, int fps) {
    ...
    ss << "Job System Load | Total - " << std::left << std::setw(5) << total_jobs << " | Breakdown - ";
    ...
    // Add padding to clear any leftover characters from the previous line
    ss << std::string(10, ' '); 
...
int main() {
    ...
    while (!glfwWindowShouldClose(g_window)) {
        ...
        JobHandle frame_handle;

        for (int i = 0; i < NUM_OBJECTS; ++i) {
            auto logic_task = [i, &object_angles, &object_speeds, delta_time]() {
                object_angles[i] += object_speeds[i] * delta_time;
            };
            job_system.submit(logic_task, JobType::AI, frame_handle);
        }
        for (int i = 0; i < NUM_OBJECTS; ++i) {
             auto render_gen_task = [i, shader_program, triangle_vao, &render_command_queue, &object_angles, &object_x_pos]() {
                Mat4 model;
            ...
            job_system.submit(render_gen_task, JobType::RenderCommandGeneration, frame_handle);
        }
        ...
        job_system.wait_for(frame_handle);

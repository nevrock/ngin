...
class Nevc : public Collection {
public:
    // Default constructor
    Nevc() = default;

    // Destructor
    virtual ~Nevc() = default;

    // Copy constructor
    Nevc(const Nevc& other) : data(other.data) {}

    // Move constructor
    Nevc(Nevc&& other) noexcept : data(std::move(other.data)) {}

    // Copy assignment operator
    Nevc& operator=(const Nevc& other) {
        if (this != &other) {
            data = other.data;
        }
        return *this;
    ...
    Nevc& operator=(Nevc&& other) noexcept {
        if (this != &other) {

...
#include <nev/texture_2d.h>
#include <nev/fileutils.h>
#include <nev/animation.h>
#include <nev/log.h>
#include <nev/mesh.h>
#include <nev/model_loader.h>
#include <nev/font.h>
...
public:
    ...
    static Shader loadShader(const std::string& name)
    {
        ...
        std::string hShaderFilePath = FileUtils::getResourcePath("shaders/" + std::string(nev::SHADER_INCLUDE_MANDATORY) + ".include");
    ...
    static ModelData& loadModelPrimitive(const std::string name) {
        ...
        if (!model) {
            ...
            Dict d;
            d.read(FileUtils::getResourcePath("nev/models/" + name + ".nevf"));
            int texIndex = 2;
    ...
    static ModelData& loadModel(const std::string& name) {
        Log::console("load model! " + name);
        std::string path = FileUtils::getResourcePath("fbx/" + name + ".fbx");
        auto& model = models[name]; // Create a new unique_ptr entry if it does not exist
        if (!model) {
            Log::console("model not found: " + name);
            model = std::make_unique<ModelData>(); // Create a new Animation if not already loaded
            model->setName(name);
        }
        if (loadModelFromFile(path.c_str(), *model)) {
            return *model;
        } else {
            throw std::runtime_error("Failed to load model: " + name);
        }
    }
    static bool loadModelFromFile(const char* path, ModelData& data) {
        Assimp::Importer importer;
        Log::console("resources load fbx: " + std::string(path));
        ...
        Log::console("scene valid: " + std::to_string(scene != nullptr));
        if (scene) {
            Log::console("scene flags: " + std::to_string(scene->mFlags));
        ...
        Log::console("model fbx loaded file: " + std::string(path));
        if (!scene) {
            Log::console("Failed to load model: " + std::string(path));
            return false;
        }
        if (scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE) {
            Log::console("The scene is incomplete and might be missing some data.");
        }
        Log::console("Number of meshes in the scene: " + std::to_string(scene->mNumMeshes));
        ...
        ModelLoader::loadAssimpModel(scene, data);

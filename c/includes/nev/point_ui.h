...
public:
    ...
    glm::mat4 getUiMatrix(glm::vec2 pos) const {
        float xScale = (size_.x)/(nev::SCREEN_WIDTH/2.0);
        float yScale = (size_.y)/(nev::SCREEN_HEIGHT/2.0);
        float xPos = (pos.x - (nev::SCREEN_WIDTH/2.0))/(nev::SCREEN_WIDTH/2.0);
        float yPos = (pos.y - (nev::SCREEN_HEIGHT/2.0))/(nev::SCREEN_HEIGHT/2.0);
